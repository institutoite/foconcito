        ...
        <div class="row">
            <div class="col-lg-12">  
                <div class="card-header border-success d-flex mt-3">
                    <h3 class="card-title">NUMEROS TELEFONICOS: <strong> {{ $empresa->empresa }} </strong> </h3>
                    <h3 class="card-title ml-auto"><a class="btn bg-cyan" href="{{route('telofempresa_crear',$empresa->id)}}"><i class="fas fa-plus-circle"></i>Agreagar mas Teléfonos</a></h3>
                </div>
                <div class="card-body"> 
                        ...
                        <tbody>
                            @foreach ($telefonos as $telefono)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('telofempresa_editar', $telefono->id)}}" class="btn-accion-tabla tooltipsC mr-2" title="Editar este número">
                                            <i class="fa fa-fw fa-edit text-primary"></i>
                                        </a>
                                        {{--@can('telofemdspresa_eliminar')--}}
                                            <form action="{{route('telofempresa_eliminar',['id' =>$telefono->id,'empresa_id'=>$empresa->id])}}" id="form{{$telefono->id}}" class="d-inline formulario" method="POST">
                                                @csrf
                                                @method("delete")
